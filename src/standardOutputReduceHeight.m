function standardOutputReduceHeight(im, numPixels, outputPrefix)
    scImage = SeamCarvingImage(im);
    [kseams, changedImage] = scImage.chooseKHorizontalSeams(im, numPixels);
    output = changedImage.image;
    osize = size(im);
    imwrite(im, [outputPrefix, '_original.jpg']);
    imwrite(output, [outputPrefix, '_seam_reduce.jpg']);
    imwrite(imresize(im, [osize(1) - numPixels, osize(2)]),[outputPrefix, '_resize.jpg']);
